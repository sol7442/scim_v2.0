...
@Slf4j
public abstract class ScimRdbService implements ScimEntityAllService, ScimStorageService {
	protected DataSource 	source;
	protected ScimStorage 	storage;
	ScimRdbResourceSchema 	schema;

	public ScimRdbService(DataSource source, ScimStorage storage) {
		this.source 	= source;
		this.storage 	= storage;
	}
	public void setResourcerSchema(ScimResourceSchema schema) {
		this.schema = (ScimRdbResourceSchema) schema;
	}
	...
	private ScimIdentity makeIdentityObject() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		return (ScimIdentity) Class.forName(schema.getIdentityClassName()).newInstance();
	}
	...
	private void create(Connection connection, ScimEntity entity) throws  ScimException, SQLException {
		...
		try {
			...
			for (Entry<String, Object> entry : entity.entrySet()) {
				ScimRdbAttributeSchema attribute = schema.getAttribute(entry.getKey());
	...
	private ScimEntity readResultSet(ResultSet result_set, ResultSetMetaData meta_data) throws ScimException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		try {
			for(int i=1; i<=meta_data.getColumnCount(); i++) {
				...
				ScimRdbAttributeSchema attribute = schema .findAttributeByColumn(column_name);
				if(attribute != null) {
